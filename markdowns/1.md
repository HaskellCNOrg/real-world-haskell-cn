...
**ghci**程序是GHC的交互式解析器。它可以让用户输入Haskell表达式并对其求值，浏览模块以及调试代码。如果你熟悉Python或是Ruby，那么ghci一定程度上和`python`，`irb`很像，这两者正是Python和Ruby的交互式解析器。
...
在类Unix系统中，我们在shell视窗下运行**ghci**。而在Windows系统下，你可以通过开始菜单找到它。比如，如果你在Windows XP下安装了GHC，你应该从"所有程序"，然后"GHC"下找到**ghci**。(参考章节[Windows]()里的截图。)
...
为了简单和一致起见，在本书中我们会用字符窜 'ghci>' 来替代ghci的默认提示符。
